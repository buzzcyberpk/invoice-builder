...
const InvoiceEditor = () => {
    ...
    const handleSave = () => {
        ...
        addToast('Invoice successfully saved to Dashboard!', 'success');
    };
    ...
    const handleDownload = async () => {
        ...
        if (!recipient) {
            addToast('Please select a client from the "To" dropdown before downloading the PDF.', 'error');
            return;
        }
        ...
        <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
                <div style={{ marginBottom: '16px' }}>
                    <select className="form-control" style={{ width: '100%', padding: '12px', fontSize: '16px', borderRadius: 'var(--border-radius-soft)', border: '1px solid var(--color-neutral-5)' }}>
                ...
                <div ref={invoiceRef} className="card" style={{ padding: '40px', backgroundColor: '#fff', boxShadow: 'var(--shadow-m)', borderRadius: 'var(--border-radius-soft)' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px' }}>
                        <div style={{ paddingRight: '20px' }}>
                            {settings?.logo ? (
                                ...
                                <img src="https://ui-avatars.com/api/?name=XS&background=4267B2&color=fff&font-size=0.5&bold=true" alt="Default Logo" style={{ width: '80px', height: '80px', borderRadius: 'var(--border-radius-soft)' }} />
                            )}
                        </div>
                    ...
                    <div style={{ display: 'flex', gap: '40px', marginBottom: '40px' }}>
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ border: '1px solid transparent', padding: '16px', background: 'var(--color-neutral-1)', borderRadius: 'var(--border-radius-soft)' }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            {!recipient ? (
                                <div style={{ border: '1px dashed var(--color-neutral-5)', padding: '16px', borderRadius: 'var(--border-radius-soft)' }}>
                                    ...
                                    <select
                                        ...
                                        style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)' }}
                                        value={recipient}
                                ...
                                (() => {
                                    ...
                                    return (
                                        <div style={{ border: '1px solid transparent', padding: '16px', borderRadius: 'var(--border-radius-soft)', background: 'var(--color-neutral-1)', position: 'relative', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                                            ...
                                            {selectedClient?.logo && (
                                                <div style={{ width: '48px', height: '48px', flexShrink: 0 }}>
                                                    <img src={selectedClient.logo} alt="Client Logo" style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 'var(--border-radius-soft)' }} />
                                                </div>
                                            )}
                    ...
                    {(showCompanyInfo || showClientInfo || showCustomField) && (
                        <div style={{ display: 'flex', gap: '24px', marginBottom: '40px', flexWrap: 'wrap', padding: '16px', backgroundColor: 'var(--color-neutral-1)', borderRadius: 'var(--border-radius-soft)' }}>
                            {showCompanyInfo && (
                                <div style={{ flex: '1 1 30%' }}>
                                    <label className="text-neutral-7 margin-bottom-s display-block">Extra Company Info</label>
                                    <input type="text" className="form-control" placeholder="Company Registration No..." style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)' }} />
                                </div>
                            )}
                            {showClientInfo && (
                                <div style={{ flex: '1 1 30%' }}>
                                    <label className="text-neutral-7 margin-bottom-s display-block">Extra Client Details</label>
                                    <input type="text" className="form-control" placeholder="Client Reference..." style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)' }} />
                                </div>
                            )}
                            {showCustomField && (
                                <div style={{ flex: '1 1 30%' }}>
                                    <label className="text-neutral-7 margin-bottom-s display-block">Custom Field</label>
                                    <input type="text" className="form-control" placeholder="PO Number..." style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)' }} />
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <div style={{ marginBottom: '40px' }}>
                                        ...
                                        <React.Fragment key={item.id}>
                                            <tr>
                                                <td style={{ padding: '8px 0' }}>
                                                    <input type="text" placeholder="Name" value={item.name} onChange={(e) => updateItem(item.id, 'name', e.target.value)} style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)' }} />
                                                </td>
                                                <td style={{ padding: '8px 0 8px 8px' }}>
                                                    <input type="number" min="1" value={item.quantity} onChange={(e) => updateItem(item.id, 'quantity', parseFloat(e.target.value) || 0)} style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)', textAlign: 'right' }} />
                                                </td>
                                                <td style={{ padding: '8px 0 8px 8px' }}>
                                                    <input type="number" min="0" value={item.price} onChange={(e) => updateItem(item.id, 'price', parseFloat(e.target.value) || 0)} style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)', textAlign: 'right' }} />
                                                </td>
                                                <td style={{ padding: '8px 0 8px 8px' }}>
                                                    <select value={item.tax} onChange={(e) => updateItem(item.id, 'tax', parseFloat(e.target.value))} style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)' }}>
                                            ...
                                            <tr>
                                                <td colSpan="6" style={{ paddingBottom: '16px', borderBottom: '1px solid var(--color-neutral-3)' }}>
                                                    <input type="text" placeholder="Description" value={item.description} onChange={(e) => updateItem(item.id, 'description', e.target.value)} style={{ width: '100%', padding: '8px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)', background: 'var(--color-neutral-1)' }} />
                                                </td>
                                            </tr>
                                        </React.Fragment>
                                    ))}
                        ...
                        <button
                            ...
                            onClick={addItem}
                            style={{ width: '100%', padding: '16px', border: '2px dashed var(--color-neutral-5)', background: 'transparent', color: 'var(--color-primary)', fontWeight: 'bold', borderRadius: 'var(--border-radius-soft)', cursor: 'pointer', textAlign: 'center' }}>
                        ...
                        <div style={{ marginBottom: '40px', display: 'flex', gap: '24px' }}>
                            {showDescription && (
                                <div style={{ flex: 1 }}>
                                    ...
                                    <textarea placeholder="General details about this invoice..." style={{ width: '100%', padding: '16px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)', minHeight: '80px', resize: 'vertical' }}></textarea>
                                </div>
                            )}
                            {showPayment && (
                                <div style={{ flex: 1 }}>
                                    <h4 className="heading4 margin-bottom-s">Payment details</h4>
                                    <textarea placeholder="Bank account, IBAN, SWIFT..." style={{ width: '100%', padding: '16px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)', minHeight: '80px', resize: 'vertical' }}></textarea>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        <textarea placeholder="Terms and conditions" style={{ width: '100%', padding: '16px', border: '1px solid var(--color-neutral-4)', borderRadius: 'var(--border-radius-soft)', minHeight: '100px', resize: 'vertical' }}></textarea>
                    </div>
